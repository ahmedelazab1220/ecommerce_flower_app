...
class AuthRepoImpl implements AuthRepo{
  ...
  Future<Result<void>> forgetPassword(ForgetPasswordRequestDto forgetPasswordRequestDto) {
    var response = _apiManager.execute(
      () async {
        final response = await _authRemoteDataSource.forgetPassword(forgetPasswordRequestDto);
        return response;
      },
    );
    return response;
  ...
  Future<Result<void>> resetPassword(ResetPasswordRequestDto resetPasswordRequestDto) {
    var response = _apiManager.execute(
          () async {
        final response = await _authRemoteDataSource.resetPassword(resetPasswordRequestDto);
        return response;
      },
    );
    return response;
  ...
  Future<Result<void>> verifyResetCode(VerifyResetCodeRequestDto verifyResetCodeRequestDto) {
    var response = _apiManager.execute(
          () async {
        final response = await _authRemoteDataSource.verifyResetCode(verifyResetCodeRequestDto);
        return response;
      },
    );
    return response;
